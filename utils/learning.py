...
class MVGLatent(CVAELatent):
    ...
    def __init__(self, latent_dim):
        ...
        self.p_dist = None          # will be filled in by LaneSwapCVAE.encoder
        self.q_dist = None          # will be filled in by LaneSwapCVAE.encoder
    ...
    def summarize_for_tensorboard(self):
        ...
        tf.summary.histogram("mu_norm_diff", tf.norm(q_mu - p_mu, axis=1))
        tf.summary.histogram("sigma_diff", q_sigma - p_sigma)
        tf.summary.histogram("q_sigma", q_sigma)
        tf.summary.histogram("p_sigma", p_sigma)
...
class DiscreteLatent(CVAELatent):
    ...
    def __init__(self, N, K):
        ...
        self.N = N
        self.K = K
        self.temp = None            # will be filled in by LaneSwapCVAE.set_annealing_params
        self.z_logit_clip = None    # may be filled in by LaneSwapCVAE.set_annealing_params
        self.p_dist = None          # will be filled in by LaneSwapCVAE.encoder
        self.q_dist = None          # will be filled in by LaneSwapCVAE.encoder
        self.z_NK = None            # will be filled in by LaneSwapCVAE.encoder (calling self.sample, below)
        self.kl_min = None          # will be filled in by LaneSwapCVAE.setup_model
    ...
    def summarize_for_tensorboard(self):
        tf.summary.histogram("p_z_x", self.p_dist.probs)
        tf.summary.histogram("q_z_xy", self.q_dist.probs)
        tf.summary.histogram("p_z_x_logits", self.p_dist.logits)
        tf.summary.histogram("q_z_xy_logits", self.q_dist.logits)
        if self.z_dim <= 9:
            for i in range(self.N):
                for j in range(self.K):
                    tf.summary.histogram("q_z_xy_logit{0}{1}".format(i,j), self.q_dist.logits[:,i,j])